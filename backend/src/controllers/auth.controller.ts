...
export const authController = {
  ...
  registerBusiness: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const expiresAt = new Schema.Types.Date(Date.now() + 24 * 60 * 60 * 1000); // 24 hours
      ...
      try {
        ...
        await sendVerificationEmail(email, verificationToken, expiresAt);
  ...
  resendVerificationEmail: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Send verification email
      await sendVerificationEmail(user.email, verificationToken, expiresAt);
