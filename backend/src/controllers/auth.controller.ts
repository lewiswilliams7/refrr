...
export const authController = {
  ...
  registerBusiness: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      try {
        ...
        await sendVerificationEmail(email, verificationToken, new Date(expiresAt.toString()).toISOString());
  ...
  forgotPassword: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Send reset email
      try {
        await sendPasswordResetEmail(email, resetToken, new Date(expiresAt.toString()).toISOString());
  ...
  resendVerificationEmail: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Send verification email
      await sendVerificationEmail(user.email, verificationToken, new Date(expiresAt.toString()).toISOString());
